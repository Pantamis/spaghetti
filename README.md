...
where `e ∈ {0,1,2}` is the endomorphism power the search applied (`λ` is the secp256k1 GLV scalar, `λ³ = 1`) and `s = ±1` fixes the y parity when the 6th char is fixed. The result is printed as a **tweak string** `<t>/<e>/<s>`, e.g. `48213946821/1/-`. The offset space `t < 2^58` is cut into `2^22` ranges of `2^36` offsets; the workers (CPU threads and the GPU) take ranges from a shared queue in order and move on to the next one when theirs is exhausted, so every published `t` is below `2^58` whatever the thread count. That bound is what makes `t` recoverable by brute force (next paragraph). All ranges together cover about `3·2^58 ≈ 2^59.6` x candidates (three per point), enough for prefixes up to 11 characters (`2^57` expected candidates, a 0.25% chance of exhausting the space without a match; 10 characters: `e^-24`); the search warns when the pattern is expected to need more than a quarter of that, and fails cleanly once every range is exhausted.
...
For each of the six `(e, s)` variants it solves `s·λ^{-e}·B − D = t·G` for `t < 2^58` with baby-step giant-step: a table of `x(j·G)` for `j < 2^K` (`--baby-bits`, default 24, 240 MB, built in about 1.5 s) and up to `2^(58−K)` giant steps per variant, walked with the same batched-inversion machinery as the search. The giant steps run in growing levels over all six variants, so a small `t` is found quickly whatever its variant. At about 200 M giant steps/s (Apple M2 Pro, 12 threads) that is up to 86 s per variant at the default `K`; the worst case (`t = 2^58 − 12345` in the last variant) measured 8 min 46 s on an Apple M3 Pro with 12 threads (196 M giant steps/s), and a `t` found by a full search takes about half of that on average. Each extra baby bit halves the giant-step work and doubles the table (`--baby-bits 26`: 1 GB, 4× faster; `--baby-bits 22`: 60 MB, 4× slower).
